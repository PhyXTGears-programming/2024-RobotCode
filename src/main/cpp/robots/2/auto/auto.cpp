...
std::vector<frc::Pose2d> robot2::loadPosePathFromJSON(wpi::json &json) {
...
std::vector<PathPoint> robot2::loadPathFromJSON(wpi::json &json, SubsystemRegistry & registry) {
...
frc2::CommandPtr robot2::loadPoseFollowCommandFromFile(Drivetrain *m_drivetrain, std::string_view filename) {
...
frc2::CommandPtr robot2::loadPathFollowCommandFromFile(std::string_view filename, SubsystemRegistry & registry) {
    std::cout << std::endl << "Robot: building path for auto from '" << filename << "'" << std::endl;
    try {
        std::error_code ec;
        std::unique_ptr<wpi::MemoryBuffer> fileBuffer =
...
frc2::CommandPtr robot2::generatePathFollowCommand(std::vector<PathPoint> && path, Drivetrain *c_drivetrain) {
    int    *currentPoseIndex  = new int(0);
    Vector *movementDirection = new Vector(0, 0);
...
frc2::CommandPtr robot2::moveBackwardsCommand(Drivetrain *c_drivetrain) {
    std::vector<frc::Pose2d> path{
...
frc2::CommandPtr robot2::moveForwardsCommand(Drivetrain *c_drivetrain) {
    std::vector<frc::Pose2d> path{
        frc::Pose2d(0.0_m, 0.0_m, 0.0_rad),
        frc::Pose2d(0.1_m, 0.0_m, 0.0_rad),
        frc::Pose2d(0.2_m, 0.0_m, 0.0_rad),
