...
robot2::Drivetrain::Drivetrain(std::shared_ptr<cpptoml::table> table) {
    double turnP = 0.0;
    double turnI = 0.0;
    double turnD = 0.0;
...
void robot2::Drivetrain::Periodic() {
...
bool robot2::Drivetrain::IsFieldOriented() {
    return m_isFieldOriented;
}

void robot2::Drivetrain::SetFieldOriented(bool isFieldOriented) {
    m_isFieldOriented = isFieldOriented;
}

void robot2::Drivetrain::ToggleFieldOriented() {
    m_isFieldOriented = !m_isFieldOriented;
}
 
void robot2::Drivetrain::ResetGyro() {
    m_gyroOffset = -m_gyro.GetYaw();
}

void robot2::Drivetrain::ResetGyroToHeading(units::radian_t heading) {
    m_gyroOffset = (heading - GetHeading()).convert<units::degree>().value();
}

radian_t robot2::Drivetrain::GetHeading() {
    return -degree_t(m_gyro.GetYaw() + m_gyroOffset);
}

void robot2::Drivetrain::UpdateOdometry() {
    m_odometry->Update(
        frc::Rotation2d(GetHeading()),
        {
            m_frontLeft->GetPosition(),
...
void robot2::Drivetrain::ResetPosition(){
...
void robot2::Drivetrain::SetPosition(radian_t heading, frc::Pose2d toPose) {
    m_odometry->ResetPosition(
...
void robot2::Drivetrain::SetPose(frc::Pose2d toPose) {
    m_odometry->ResetPosition(
        frc::Rotation2d(GetHeading()),
        {
            m_frontLeft->GetPosition(),
...
Point robot2::Drivetrain::GetChassisPosition() {
